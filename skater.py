...
class Spanning_Forest(object):
    def __init__(self,
                 ...
                 center=np.mean):
        """
        dissimilarity : a distance metric
        affinity : an affinity metric between 0,1. 
    ...
    def fit(self, n_clusters, W, 
        ...
        W : pysal W object
        data: np.ndarray of (N,P) shape with N obs and P features
    ...
    def find_cut(self, MSF, data=None, quorum=-np.inf, 
        ...
        for in_node, out_node in np.vstack(MSF.nonzero()).T: # iterate over MSF edges
            if zero_in:
    ...
    def make_cut(self, in_node, out_node, score, MSF=None):
        ...
        if a deletion namedtuple is passed, it overrides the in_node, out_node, and score proi
        """
        if MSF is None:
